...
int main(int argc, char ** argv) {
    ...
    for (const auto & inst : params_instances) {
        ...
        if (!params.no_warmup) {
            ...
            uint64_t t_warmup_end = get_time_ns();
            printf("Warmup: E2E: %.6fs, TTFT: %.6fs, TPOT: %.6fs\n",
                1. * (t_warmup_end - t_warmup_start) / 1e9,
                1. * (t_warmup_ttft - t_warmup_start) / 1e9,
