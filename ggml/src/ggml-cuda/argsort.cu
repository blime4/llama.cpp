...
static void argsort_f32_i32_cuda_cub(ggml_cuda_pool & pool,
    ...
    CUDA_CHECK(cudaMemcpyAsync(temp_keys, x, ncols * nrows * sizeof(T), cudaMemcpyDeviceToDevice, stream));
...
static __global__ void k_argsort_f32_i32(const T * x, int * dst, const int ncols, int ncols_pad) {
    // bitonic sort
    int col = threadIdx.x;
    int row = blockIdx.x;
...
template <typename T>
static void argsort_f32_i32_cuda_bitonic(const T *       x,
                                         int *           dst,
                                         const int       ncols,
                                         const int       nrows,
                                         ggml_sort_order order,
