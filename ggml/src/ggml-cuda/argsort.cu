...
// F16 version of argsort kernel - converts to F32 for comparison
template<ggml_sort_order order>
static __global__ void k_argsort_f16_i32(const half * x, int * dst, const int ncols, int ncols_pad) {
    // bitonic sort
    int col = threadIdx.x;
    int row = blockIdx.x;
        ...
        for (int j = k / 2; j > 0; j /= 2) {
            ...
            if (ixj > col) {
                if ((col & k) == 0) {
                    if (dst_row[col] >= ncols ||
                        (dst_row[ixj] < ncols && (order == GGML_SORT_ORDER_ASC ?
                            __half2float(x_row[dst_row[col]]) > __half2float(x_row[dst_row[ixj]]) :
                            __half2float(x_row[dst_row[col]]) < __half2float(x_row[dst_row[ixj]])))
                    ) {
                        ggml_cuda_swap(dst_row[col], dst_row[ixj]);
                    }
...
void argsort_f16_i32_cuda_bitonic(const half *    x,
    ...
    if (order == GGML_SORT_ORDER_ASC) {
        k_argsort_f16_i32<GGML_SORT_ORDER_ASC>
            <<<block_nums, block_dims, shared_mem, stream>>>(x, dst, ncols, ncols_pad);
    } else if (order == GGML_SORT_ORDER_DESC) {
        k_argsort_f16_i32<GGML_SORT_ORDER_DESC>
            <<<block_nums, block_dims, shared_mem, stream>>>(x, dst, ncols, ncols_pad);
    } else {
        GGML_ABORT("fatal error");
    }
...
void ggml_cuda_op_argsort(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (src0->type == GGML_TYPE_F16) {
        argsort_f16_i32_cuda_bitonic((const half *)src0_d, dst_d, ncols, nrows, order, stream);
    } else {
#ifdef GGML_CUDA_USE_CUB
        const int    ncols_pad      = next_power_of_2(ncols);
        const size_t shared_mem     = ncols_pad * sizeof(int);
        ...
        if (shared_mem > max_shared_mem || ncols > 1024) {
            ...
            argsort_f32_i32_cuda_cub(pool, (const float *)src0_d, dst_d, ncols, nrows, order, stream);
        } else {
            argsort_f32_i32_cuda_bitonic((const float *)src0_d, dst_d, ncols, nrows, order, stream);
        }
#else
        argsort_f32_i32_cuda_bitonic((const float *)src0_d, dst_d, ncols, nrows, order, stream);
#endif
