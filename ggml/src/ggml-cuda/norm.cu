...
static __global__ void rms_norm(
    ...
    for (int col = tid; col < ncols; col += block_size) {
        dst[col] = static_cast<T>(scale * static_cast<float>(x[col]));
    }
}
...
static void rms_norm_cuda(
    ...
    if (ncols < 1024) {
        const dim3 block_dims(WARP_SIZE, 1, 1);
        rms_norm<T, WARP_SIZE><<<blocks_num, block_dims, 0, stream>>>(x, dst, ncols, stride_row, stride_channel, stride_sample, eps);
    } else {
        const dim3 block_dims(1024, 1, 1);
        rms_norm<T, 1024><<<blocks_num, block_dims, 0, stream>>>(x, dst, ncols, stride_row, stride_channel, stride_sample, eps);
    }
}
