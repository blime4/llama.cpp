...
#ifdef GGML_USE_DLFA
        // DL: chunk prefill, CUDA graph does not check the node's batch size.
        if (!cgraph->flash_attn && node->op == GGML_OP_ADD &&
...
#ifndef GGML_USE_DLCU // DL-TODO. support ggml_cuda_op_rms_norm_fused_add fp16 impl.
...
#ifndef GGML_USE_DLFA // DL-TODO: remove flash_attn parameter once DL-FA is fully integrated.
static bool ggml_cuda_set_cuda_graph_enabled(ggml_backend_cuda_context * cuda_ctx) {
#else
static bool ggml_cuda_set_cuda_graph_enabled(ggml_backend_cuda_context * cuda_ctx, const ggml_cgraph * cgraph) {
#endif
#ifdef USE_CUDA_GRAPH
...
static void ggml_backend_cuda_graph_optimize(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    #ifdef GGML_USE_DLFA // DL-TODO: remove flash_attn parameter once DL-FA is fully integrated.
    const bool use_cuda_graph = ggml_cuda_set_cuda_graph_enabled(cuda_ctx, cgraph);
    #else
