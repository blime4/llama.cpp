...
| 1.3 | 2026-03-10 | 添加调查结果：残差深度卷积转置修复，ZCR质量指标 |
| 1.4 | 2026-03-10 | **所有问题已解决**: 输出卷积、ConvTranspose1D、解码器层修复完成 |

---

## 10. 调查更新 (2026-03-10) - 已解决
...
**问题:** 残差单元深度卷积中存在不必要的转置操作，导致音频数据损坏。

**影响:**
| 指标 | 修复前 | 修复后 |
|------|--------|--------|
| ZCR (短音频) | 0.49 (噪声) | 0.045 (语音) |
| 音频质量 | 白噪声 | 正常语音 |

#### 问题 2: 输出卷积内核格式错误 (新修复)
**问题:** 输出卷积使用 `ggml_im2col` 时需要内核格式 `[K, IC, 1, OC]`，但收到了错误格式的数据。

**修复措施:**
- 修复内核格式为 `[K, IC, 1, OC]` 以兼容 ggml_im2col
- 添加输入张量的 F32 转换 (CUDA im2col 需要 F32)
- 修复矩阵乘法顺序为 `mul_mat(im2col_2d, kernel_2d)`

**影响:** 长音频 ZCR 从 0.155 降至 0.057 (失真减少 63%)

#### 问题 3: ConvTranspose1D 内核类型错误 (新修复)
**问题:** CUDA `ggml_conv_transpose_1d` 需要 F32 内核，但传递了 F16。

**修复措施:**
- 将内核类型转换从 F16 改为 F32
- 添加内核置换从 `[OC, K, IC]` 到 `[K, OC, IC]`

#### 问题 4: 解码器层深度卷积修复 (新修复)
**问题:** 残差深度卷积内核需要 F16 转换以保持一致性。

**修复措施:**
- 为解码器层中的深度卷积内核添加 F16 转换

### 10.2 最终音频质量状态

| 音频时长 | ZCR 修复前 | ZCR 修复后 | 状态 |
|----------|------------|------------|------|
| 短音频 (<2s) | 0.045 | 0.045-0.057 | ✓ 良好 |
| 长音频 (>10s) | 0.155 | 0.057 | ✓ 良好 (已修复!) |
| 目标值 | 0.02-0.06 | 0.02-0.06 | ✓ 已达成 |
...
|------|----------|------|
| 内核分析 | 7个内核布局 | ✓ 修复后验证正确 |
| Snake激活 | Epsilon差异 | ✓ 影响可忽略 |
| 张量比较 | 逐层比较 | ✓ 识别出输出卷积问题 |
| 长度分析 | 时长 vs ZCR | ✓ 确认与长度无关 |
...
./build/bin/llama-orpheus-tts \
    ...
    --model-vocoder models/snac-24khz-f16.gguf \
    -p "这是一段测试文本，用于验证 Orpheus TTS 系统的语音合成质量。" \
    -o test_tts.wav
```
